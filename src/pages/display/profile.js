    ...
    <Container>
      <Padding>
        <div className={styles.profile}>
          <Identicon address={user.wallet} logo={user.identicon} />

          <div className={styles.info}>
            <p>
              <strong>{user.subjkt || user.alias}</strong>
            </p>

            {user.description && <p>{user.description}</p>}

            <Button href={`https://tzkt.io/${user.wallet}`}>
              <Primary>{walletPreview(user.wallet)}</Primary>
            </Button>

            <div>
              {user.twitter && (
                <Button href={`https://twitter.com/${user.twitter}`}>
                  <VisuallyHidden>{`https://twitter.com/${user.twitter}`}</VisuallyHidden>
                  <svg
              ...
              {user.tzprofile && (
                <Button href={`https://tzprofiles.com/view/${user.tzprofile}`}>
                  <VisuallyHidden>{`https://tzprofiles.com/view/${user.tzprofile}`}</VisuallyHidden>
                  <svg
                    height="16"
                    viewBox="0 0 16 16"
                    width="16"
              ...
              {user.discord && (
                <Button onClick={setDiscordCopied}>
                  <Primary>
                    <span
                    ...
                    >
                      <VisuallyHidden>{`${user.discord}`}</VisuallyHidden>
                      <svg
                        width="20"
                        height="15"
                        viewBox="0 0 20 15"
              ...
              {user.github && (
                <Button href={`https://github.com/${user.github}`}>
                  <VisuallyHidden>{`https://github.com/${user.github}`}</VisuallyHidden>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
              ...
              {user.dns && (
                <Button href={`http://${user.dns}`}>
                  <VisuallyHidden>{user.dns}</VisuallyHidden>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
