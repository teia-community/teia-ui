...
export const BeneficiariesUI = ({
  ...
  if (multilineContent.length) {
    const currentAddresses = beneficiaries
      .filter((c) => c.address)
      .map((c) => c.address)
            ...
            {beneficiaries.map((beneficiary, index) => {
              return (
                <BeneficiaryRow
                  ...
                  onUpdate={(beneficiaryData) =>
                    onUpdate(index, beneficiaryData)
                  }
                  ...
                  onSelectPercentage={
                    noCollaborators
                      ? null
                      : (percentage) => onSelectPercentage(index, percentage)
                  }
                />
              )
            })}
        ...
        <div className={styles.mt2}>
          ...
          {totalParticipants < 2 && (
            <p className={styles.muted}>
              You need to have at least two participants to create a
