  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
      ...
      <div style={{ textAlign: 'left', marginTop: '20px', padding: '10px', border: '1px solid #ddd' }}>
        ...
        <ul>
          <li>Right to Reproduce: {customLicenseData?.clauses.reproduce ? '✅ Yes' : '🚫 No'}</li>
          <li>Right to Broadcast: {customLicenseData?.clauses.broadcast ? '✅ Yes' : '🚫 No'}</li>
          <li>Right to Public Display: {customLicenseData?.clauses.publicDisplay ? '✅ Yes' : '🚫 No'}</li>
          <li>Right to Create Derivative Works: {customLicenseData?.clauses.createDerivativeWorks ? '✅ Yes' : '🚫 No'}</li>
          <li>Exclusive Rights: {customLicenseData?.clauses.exclusiveRights}</li>
          <li>Creator Retains Rights: {customLicenseData?.clauses.retainCreatorRights ? '✅ Yes' : '⚠️ No'}</li>
          <li>Rights Are Transferable: {customLicenseData?.clauses.rightsAreTransferable ? '✅ Yes' : '🚫 No'}</li>
          <li>Clauses Have Expiration Date: {customLicenseData?.clauses.expirationDateExists ? '✅ Yes' : '🚫 No'}</li>
          {customLicenseData?.clauses.expirationDateExists && (
            <li>Expiration Date: {customLicenseData.clauses.expirationDate || 'None'}</li>
          )}
          <li>Release to Public Domain: {customLicenseData?.clauses.releasePublicDomain ? '✅ Yes' : '🚫 No'}</li>
          <li>Custom URI Enabled: {customLicenseData?.clauses.customUriEnabled ? '✅ Yes' : '🚫 No'}</li>
      ...
      <div style={{ border: '1px solid #ccc', padding: '15px', marginTop: '15px' }}>
        ...
        >{customLicenseData?.documentText || 'No document text available.'}</pre>
      </div>
