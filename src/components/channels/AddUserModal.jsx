...
/**
 * Add a wallet to a channel as either an Admin (full mod, creator-only) or
 * a User (Merkle allowlist member, creator-or-admin). Admin adds happen
 * directly via update_channel_admins; user adds rebuild the Merkle list
 * via addMerkleUsers.
 *
 * Closed channels have no Merkle path, so the User role is hidden and only
 * admin adds (creator-only) are available.
 *
 * Props:
 * - channelId
 * - currentMerkleList — the addresses already in the Merkle list (may be empty)
 * - canAddAdmin — whether the viewer is the channel creator
 * - accessMode — current channel access mode; 'closed' disables the User role
 * - onClose — close the modal
 * - onAdded — refetch hooks after successful add
 */
