...
export default function ChannelSettings() {
  ...
  const handleSaveAccess = async () => {
    ...
    try {
      ...
      if (accessMode === 'allowlist') {
        merkleAddresses = allowlistText
          .split(/[\n,]+/)
          .map((a) => a.trim())
          .filter(Boolean)
        const invalid = merkleAddresses.filter((a) => !isTzAddress(a))
        if (invalid.length > 0) {
          alert(`Invalid address(es): ${invalid.join(', ')}`)
          setSaving(false)
          return
        }
      }
  ...
  const handleAddAdmin = async () => {
    ...
    try {
      ...
      console.warn('Admin add failed:', e)
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleRemoveAdmin = async (admin) => {
    ...
    try {
      ...
      console.warn('Admin remove failed:', e)
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleRemoveMerkleUser = async (user) => {
    ...
    try {
      ...
      console.warn('Remove user failed:', e)
    } finally {
      setSaving(false)
    }
  }
