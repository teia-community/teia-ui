...
/** Format a Date back to the shape of `template` (date-only vs datetime-local). */
 ...
 *
 * @param {string} startStr  the series anchor ('YYYY-MM-DD[THH:mm]')
 * @param {string} freq
...
export function occurrenceAt(startStr, freq, interval, k) {
  const anchor = parseLocal(startStr)
  const n = Math.max(1, Number(interval) || 1) * k
  if (freq === 'DAILY' || freq === 'WEEKLY') {
    const d = new Date(anchor)
    d.setDate(d.getDate() + (freq === 'WEEKLY' ? 7 * n : n))
...
export function expandOccurrences(
  ...
  const durMs = endDate ? parseLocal(endDate) - parseLocal(startDate) : null
  const out = []
  ...
  for (let k = 0; k < HARD_CAP; k++) {
    ...
    if (cursor.slice(0, 10) >= windowStart) {
      const occEnd =
        durMs != null
          ? formatLike(new Date(parseLocal(cursor).getTime() + durMs), endDate)
          : ''
